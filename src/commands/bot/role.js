    ...
    async run(client, message) {
        ...
        servers.findOne({
            ...
            if (!s) {
                ...
                return message.chanel.send(`This server does not have a role system set up! Please contact 🌠┊𝑃𝑜𝑤𝑒𝑟𝑠𝑡𝑎𝑟™#4121 to get it set up: ${client.settings.server}`);
            }
            else if (s.roleSystem) {
                const voted = await getVoted(client, message.author);
                if (!voted) {
                    const voteEmbed = new Discord.MessageEmbed()
                        .setDescription('You must **vote** to get this role. You can vote [here](https://top.gg/bot/772133876813922314/vote)')
                        .setColor(client.colors.main);
                    return message.channel.send(voteEmbed);
                }
                else {
                    const member = message.member;
                    const role = message.guild.roles.cache.get('789832305928568832');
                ...
                return message.channel.send(`This server does not have a role system set up! Please contact 🌠┊𝑃𝑜𝑤𝑒𝑟𝑠𝑡𝑎𝑟™#4121 to get it set up: ${client.settings.server}`);
            }
        });
    }
};