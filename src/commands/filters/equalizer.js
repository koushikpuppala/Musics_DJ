...
module.exports = class Equalizer extends Command {
	...
	async run(client, message, args) {
		...
		for (let i = 0; i < bands.length; i++) {
			if (i > 13) break;
			if (isNaN(bands[i])) return message.channel.send(`Band #${i + 1} is not a valid number. Please type \`+ eq\` for info on the equalizer command.`);
			if (bands[i] > 10) return message.channel.send(`Band #${i + 1} must be less than or equal to 10. Please type \`+ eq\` for info on the equalizer command.`);
		}
		...
		const embed = new Discord.MessageEmbed()
			...
			.setFooter('To reset the equalizer type: + reset')
			.setColor(client.colors.main);
		await delay(5000);
