	...
	async run(message) {
		...
		servers.findOne({
			...
			async function runCommand(client) {
				...
				if(message.guild.id != '775411681714503680') {
				...
				if (cmd.voteLocked == true && await premium(message.author.id, 'Premium') == false && await premium(message.author.id, 'Pro') == false) {
					...
					if (!voted) {
						const voteEmbed = new Discord.MessageEmbed()
							.setDescription('You must **vote** to use this command. **You can vote [here](https://top.gg/bot/772133876813922314/vote)**.\nYou can bypass vote locked commands by purchasing premium [here](https://www.patreon.com/Koushik_5).')
